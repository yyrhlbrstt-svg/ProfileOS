...
FAMILIES: tuple[OpeningFamily, ...] = (
    ...
    OpeningFamily(
        "tilt_slide", "הזזה נטויה (קיפ-הזזה)", "window", "sliding",
        leaves=(2,), widths=(1800, 2400, 3000), heights=(1400, 2100),
        sill=200,
        note="הכנף נוטה לאוורור ואז מוסטת — פרזול ייעודי, לא פרזול הזזה רגיל",
        fittings="dwelling",
        tags=("קיפ הזזה", "הזזה נטויה", "tilt slide", "אוורור", "מרפסת"),
        min_width=1200, max_width=6000, max_height=2600,
    ),
    OpeningFamily(
        "folding", "דלת אקורדיון (הרמונית)", "door", "door",
        leaves=(2, 3, 4, 5, 6), sash_position="all",
        widths=(2400, 3000, 3600, 4800, 6000), heights=(2100, 2400),
        sill=0,
        note="נפתחת לגמרי לצד — כל כנף נושאת את משקלה על המסילה העליונה",
        fittings="dwelling_manual",
        tags=("אקורדיון", "הרמונית", "מתקפלת", "bifold", "folding", "מרפסת"),
        min_width=1600, max_width=9000, min_height=1800, max_height=2800,
    ),
    OpeningFamily(
        "balustrade", "מעקה זכוכית", "window", "fixed",
        leaves=(1,), sash_position="none",
        widths=(1000, 1500, 2000, 3000), heights=(1000, 1100, 1200),
        sill=0, glass="lam-66-2",
        note="מעקה — הזכוכית היא רכיב בטיחות נושא, לא זיגוג",
        tags=("מעקה", "מרפסת", "גדר זכוכית", "balustrade", "בטיחות"),
        min_width=400, max_width=6000, min_height=900, max_height=1500,
    ),
    OpeningFamily(
        "mamad_door", "דלת ממ״ד", "door", "door",
        leaves=(1,), widths=(800, 900, 1000), heights=(2000, 2100),
        sill=0,
        note="דלת הדף — היא מוצר מאושר של יצרן מורשה ואינה מיוצרת כאן",
        tags=("ממד", "דלת הדף", "מיגון", "פיקוד העורף"),
        min_width=700, max_width=1200, min_height=1900, max_height=2200,
    ),
    OpeningFamily(
        "corner", "חלון פינתי ללא עמוד", "window", "fixed",
        leaves=(2,), sash_position="none",
        widths=(1600, 2000, 2600), heights=(1200, 1400, 1800),
        sill=900,
        note="שתי זכוכיות נפגשות בפינה בלי פרופיל — הזיגוג נושא, ודורש אישור",
        fittings="sill_only",
        tags=("פינה", "פינתי", "ללא עמוד", "corner", "זיגוג מבני"),
        min_width=800, max_width=5000,
    ),
    OpeningFamily(
        "bars", "סורגים", "window", "fixed",
        leaves=(1,), sash_position="none",
        widths=(800, 1000, 1200, 1600), heights=(900, 1200, 1400),
        sill=900,
        note="סורג — מידה לפי הפתח הבנוי, לא לפי מסגרת החלון",
        tags=("סורג", "סורגים", "מיגון", "פריצה", "bars"),
        max_width=3000,
    ),
    OpeningFamily(
        "automatic_door", "דלת הזזה אוטומטית", "door", "sliding",
        leaves=(1, 2), sash_position="all",
        widths=(1800, 2400, 3000), heights=(2100, 2400),
        sill=0,
        note="המנוע והבקר הם מוצר של ספק — נמדדים ונרכשים, לא נגזרים כאן",
        tags=("אוטומטית", "חיישן", "כניסה", "מסחרי", "automatic"),
        min_width=1400, max_width=6000, min_height=1900,
    ),
    OpeningFamily(
